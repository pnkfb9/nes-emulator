...
struct EncodingParams
{
    ...
    unsigned ShortLengthMod() const { return 0x100 / (1 + short_threshold); }
    ...
    unsigned ShortLengthMax() const { return shortmatchmin + ShortLengthMod() - 1; }
    unsigned ShortLongDelta() const { return shortmatchmin + ShortLengthMod() - 2; }
    size_t   OffsConstPlusIndex() const
    {
        size_t result = 0;
        if (offsconstmask == 5 && constbyte < 2 && constbyte2 == 0x100)
            ++result;
        return result;
    }
...
namespace
{
static size_t MatchingLength(const unsigned char* in1, size_t size, const unsigned char* in2)
{
    ...
    while (size >= intsize && p1[len] == p2[len])
    {
        ++len;
        size -= intsize;
    }
    len *= intsize;
    while (size > 0 && in1[len] == in2[len])
    {
        --size;
        ++len;
    }
...
static unsigned GammaScore(size_t value)
{
    ...
    while (highest_bit != 0 && !(value & highest_bit))
        highest_bit >>= 1;
...
static unsigned Cost(size_t                offset,
                     size_t                length,
                     size_t                prev_offset,
                     size_t                default_cost,
                     const EncodingParams& params)
{
    ...
    if (length >= params.shortmatchmin && length <= params.ShortLengthMax() && offset >= 1
        && offset <= params.short_threshold)
    {
        // Penalty of a short match
        return 3 + 8;
    }
    // Penalty of a long match
    if (offset <= params.short_threshold && length < 2 + params.shortmatchmin)
...
private:
    size_t
    TestMatch(const std::vector<unsigned char>& source, size_t pos1, size_t pos2, size_t limit)
    {
        if (pos2 > pos1)
        {
            size_t tmp = pos1;
    ...
    void CreateAuxLiteralTables(const unsigned LiteralUsageTable[256])
    {
        ...
        for (unsigned j = 1; j < NumUsedLiterals; ++j)
        {
            ...
            for (i = j;
                 i >= 1
                 && LiteralUsageTable[LiteralsInOrderOfCommonness[i - 1]] < LiteralUsageTable[k];
                 --i)
    ...
    void AnalyzeSource(const std::vector<unsigned char>& source, const EncodingParams& params)
    {
        ...
        {
            unsigned default_cost =
                params.useliteraltable ? 1 + GammaScore(LiteralIndices[source[a]] + 2) : 9;
            ...
            if (source[a] == params.constbyte)
                ...
                for (size_t offset = 1; a >= offset && offset <= params.MaxByteCopyOffset();
                     ++offset)
        ...
        if (!params.useliteraltable)
        {
            ...
            for (unsigned tokenno = 0; tokenno < sizeof(MagicWords) / sizeof(*MagicWords);
                 ++tokenno)
            {
                ...
                {
                    void* p = memmem(&source[begin], source.size() - begin, token, token_size);
                    if (!p)
                        break;
                               ...
                               const EncodingParams&             params)
    {
        ...
        while (a < endpos)
        {
            if (recursion_limit == 2 && max_recursion == 3
                && (a >= make_fast_when || ++n_actions >= 2))
                recursion_limit -= 1;
            if (recursion_limit == 1 && max_recursion > 1
                && (a >= make_fast_when || ++n_actions >= 4))
                recursion_limit -= 1;
            ...
            if (!recursion_limit)
            {
                ...
                if (best_eat > 1 && best_eat < params.shortmatchmin
                    && a - best_source <= params.short_threshold)
                    best_eat = 1;
                if (best_eat == 2 && params.limit_at_1byte
                    && a - best_source >= params.limit_at_1byte)
                    best_eat = 1;
                if (best_eat == 3 && params.limit_at_2byte
                    && a - best_source >= params.limit_at_2byte)
                    best_eat = 1;
    ...
    firstprivate(a, recursion_limit, max_recursion, test_last)
                ...
                for (size_t eating_count = best_eat; eating_count >= 1; --eating_count)
                {
                    if (eating_count < 2)
                    {
                        ...
                        unsigned trailing_cost =
                            cheapest_cost
                            + DetermineDecision(source,
                                                a + eating_count,
                                                eating_count >= 2 && params.appends
                                                    ? a - cheapest_source
                                                    : test_last,
                                                recursion_limit - 1,
                                                max_recursion,
...
#pragma omp flush(best_cost, best_eat, best_source, my_cost)
                       ...
                       && eating_count >= best_eat / 2*/)
                    {
                        for (size_t test = a - std::min(a, MaxDistance); test < a; ++test)
                        {
                            if (eating_count < params.shortmatchmin
                                && a - test <= params.short_threshold)
                                continue;
                            if (TestMatch(source, test, a, eating_count) >= eating_count)
                            {
                                ...
                                unsigned cheapest_cost =
                                    Cost(a - test, eating_count, test_last, DefaultCost[a], params);
...
#pragma omp flush(best_cost, best_eat, best_source, my_cost)
            ...
            if (best_eat >= 2 && params.appends)
                test_last = a - best_source;
            a += best_eat;
            total_cost += my_cost;
        }
...
public:
    void Compress(const std::vector<unsigned char>& source,
                  unsigned                          max_recursion,
                  const EncodingParams&             params)
    {
        const size_t endpos = source.size();
            ...
            for (;;)
            {
                ...
                for (size_t p = 0; p < endpos; ++p)
                {
                    ...
                    if (3 + params.literalbits < default_cost && !bomb[p])
                    {
                        ...
                        if (byte == params.constbyte || byte == params.constbyte2
        ...
        while (readpos < endpos)
        {
            ...
            if (copy_length < 2)
            {
                ...
                unsigned      default_cost =
                    params.useliteraltable ? 1 + GammaScore(LiteralIndices[byte] + 2) : 9;

                /*fprintf(stderr, "[%u]: literal %02X\n", (unsigned)readpos, byte);*/
                if (3 + params.literalbits > default_cost || bomb[readpos])
            ...
            else if (copy_length >= params.shortmatchmin && copy_length <= params.ShortLengthMax()
            ...
            else
            {
                ...
                {
                    fprintf(stderr,
                            "offset=%u, copy_length=%u, short=%u..%u, long=%u..\n",
                            (unsigned)my_offset,
    ...
    void Uncompress(const std::vector<unsigned char>& source,
                    const EncodingParams&             params,
...
#define GetBit()                                                                                   \
    (((tagpos % 8 ? (tag) << (tagpos++ % 8) : (tag = source[inpos++]) << (tagpos++ % 8)) & 0x80)   \
     >> 7)
        ...
        for (;;)
        {
            ...
            if (!GetBit()) // 10, codepair
            {
                ...
                do
                    result.push_back(result[result.size() - last_offs]);
                while (--cx > 0);
            }
            else if (!GetBit()) // 110, bytecopy
            {
                ...
                if (params.offsconstmask == 5 && params.constbyte < 2 && params.constbyte2 == 0x100)
                {
                    if (!bx)
                    {
                        result.push_back(params.constbyte);
                        continue;
                    }
                    --bx;
                    if (!bx)
                    {
                        result.push_back((result.back() + 1) & 0xFF);
                        continue;
                    }
                    --bx;
                    if (!bx)
                    {
                        result.push_back((result.back() + 2) & 0xFF);
                        continue;
                    }
                    --bx;
                }
                else
                {
                    ...
                    if (params.constbyte2 != 0x100)
                    {
                        if (!bx)
                        {
                            result.push_back(params.constbyte2);
                            continue;
                        }
                        --bx;
                    }
        ...
        for (size_t a = 0; a + 3 <= result.size() && n_calls > 0; ++a)
        {
            ...
            else if ((params.call_mask & 4) && result[a] == 0x0F)
            {
                a += 2;
                unsigned offset = result[a] | (result[a + 1] << 8);
                offset -= (a + ORG);
...
private:
    // Primitives:
    void PrimSendByte(unsigned char value) { result.push_back(value); }
    void PrimSendBit(unsigned char bit)
    {
         ...
         * BitsPosition,result[BitsPosition]);*/
    }
    void PrimSendGamma(unsigned value) // The smallest value it can encode is 2.
    {
        ...
        if (GammaSummary.size() <= n_bits_sent)
            GammaSummary.resize(n_bits_sent + 1);
        GammaSummary[n_bits_sent] += 1;
        if (GammaSummary2.size() <= origvalue)
            GammaSummary2.resize(origvalue + 1);
        GammaSummary2[origvalue] += 1;
    }

public:
    // Actual encodes
    void SendLongMatch(unsigned offset,
                       // Offset: 0..n
                       unsigned len,
                       // Len, if offset:
                       //    0..7F:    3..any (1 + 2)
    ...
    void
    SendShortMatch(unsigned offset /*1..127*/, unsigned len /*2..3*/, const EncodingParams& params)
    {
...
std::vector<unsigned char> GetAsm(const EncodingParams& params,
                                  ...
                                  const unsigned char*  LiteralTable = 0,
...
#pragma omp critical(asmlock)
    {
        ...
        std::fprintf(
            ...
            "		jnc .fourbits\n"
            "\n"
            "%%if (offsconstmask == 5 && (constbyte == 0x100 || constbyte < 2) && constbyte2 == "
            "0x100)\n"
            "	%%if constbyte == 0\n"
            "		jz STORE\n"
            "		dec ax\n"
            ...
            "	%%if ((offsconstmask & 1) + ((offsconstmask & 2) >> 1) + ((offsconstmask & 4) >> "
            "2)) > 1\n"
            "		lea bx,[di-1] ;However, 3+1+1 is better than 3+3, and 3+1+1+1 is way better "
            "than 3+3+3.\n"
            "		%%undef get_prev\n"
            ...
            "				inc ax\n"
            "				jmp short .inc_one_store ; This is 3 bytes in total, whereas mov "
            "al;jmp would be 4.\n"
            ...
            "			inc ax\n"
            "			jmp short .inc_one_store ; This is 3 bytes in total, whereas mov al;jmp "
            "would be 4.\n"
...
#pragma omp parallel for schedule(dynamic, 1)
    for (unsigned progress = 0; progress < 8 * 2 * 8 * 1 * 4 * 8 * 1 * 1 * 2 * 3 * 1 * 3 * 3;
         ++progress)
    {
        unsigned       tv = progress;
        EncodingParams test_params;
        test_params.call_mask = tv % 8;
        tv /= 8;
        test_params.appends = tv % 2;
        tv /= 2;
        test_params.limit_at_1byte = (tv % 8) << 8;
        tv /= 8;
        test_params.limit_at_2byte = 0; //(tv%12)<<8;  tv/=12;
        test_params.literalbits = tv % 4 + 2;
        tv /= 4;
        test_params.offsconstmask = tv % 8;
        tv /= 8;
        static const unsigned cb[3] = {0, 1, 0x100};
        test_params.constbyte = cb[tv % 3];
        tv /= 3; // tv%0x101; tv/=0x101;
        test_params.constbyte2 = cb[tv % 3];
        tv /= 3; // tv%0x101; tv/=0x101;
        test_params.useliteraltable = 1 - tv % 2;
        tv /= 2;
        test_params.shortmatchmin = 2 + tv % 3;
        tv /= 3;
        test_params.short_threshold = 127; // 0x7F + 0x80*(tv%2); tv/=2;
        // test_params.short_threshold = 1 + tv%0xFF; tv/=0xFF;
        test_params.n_calls = 0;

        if (test_params.constbyte == 0x100 && test_params.constbyte2 != 0x100)
            continue;
        if (test_params.constbyte != 0x100 && test_params.constbyte2 < test_params.constbyte)
            continue;
        ...
        if (test_params.call_mask != 1)
            continue;
        if (!test_params.appends)
            continue;
        if (test_params.limit_at_1byte != 0x500 && test_params.limit_at_1byte != 0x600)
            continue;
        // if(test_params.literalbits == 5) continue;
        if (test_params.literalbits != 4 && test_params.literalbits != 3)
            continue;
        if (test_params.useliteraltable)
            continue;
        // if(!(test_params.offsconstmask & 1)) continue;
        if(test_params.offsconstmask != 0/*
        && test_params.offsconstmask != 1*/)
            continue;
        // if(test_params.offsconstmask != 5) continue;
        if (test_params.shortmatchmin != 2)
            continue;
        ...
        if (!!(test_params.offsconstmask & 1) + !!(test_params.offsconstmask & 2)
                + !!(test_params.offsconstmask & 4) + (test_params.constbyte != 0x100)
...
#if 1
        bool byte1ok = test_params.constbyte == 0x00 || test_params.constbyte == 0x01
        ...
        bool byte2ok = test_params.constbyte2 == 0x00 || test_params.constbyte2 == 0x01
        ...
        */
        ...
        for (size_t a = 0; a + 3 <= buffer.size(); ++a)
        {
            ...
            else if (((test_params.call_mask & 4) && buffer[a] == 0x0F))
            {
                a += 2;
                unsigned offset = buffer[a] | (buffer[a + 1] << 8);
                offset += (a + ORG);
        ...
        std::fprintf(
            stderr,
            "Trying constbyte=%02X:%02X,literal=%d,callmask=%u(%u "
            "calls),appends=%u,limit1=%X;2=%X,offsconst=%u,table=%u(%u),shortmin=%u/%u ...\n",
            test_params.constbyte,
            test_params.constbyte2,
            ...
            test_params.useliteraltable,
            0,
            test_params.shortmatchmin,
        ...
        */
        ...
        std::vector<unsigned char> stub = GetAsm(test_params,
                                                 packer.NumUsedLiterals,
                                                 packer.LiteralsInOrderOfCommonness,
                                                 packer.result.size());
...
#pragma omp flush(params, best_packer, best_asm_skip, best_buffer)
            ...
            if (!res_size || test_size <= res_size)
            {
                std::fprintf(
                    stderr,
                    "%u+%u=%u bytes for constbyte=%02X:%02X,literal=%d,callmask=%u(%u "
                    "calls),appends=%u,limit1=%X;2=%X,offsconst=%u,table=%u(%u),shortmin=%u/%u\n",
                ...
                if (decompress != buffer_orig)
                {
                    ...
                    fprintf(stderr,
                            "--DECOMPRESSION FAILURE (%u<->%u bytes, %u bytes differ)\n",
                            (unsigned)buffer_orig.size(),
                            (unsigned)decompress.size(),
                            (unsigned)ndiff);
                }
            }
#pragma omp flush(params, best_packer, best_asm_skip, best_buffer)
    ...
    if (buffer_orig.size() < 2000)
    {
        ...
        std::vector<unsigned char> stub =
            GetAsm(params, best_packer.NumUsedLiterals, best_packer.LiteralsInOrderOfCommonness);
        best_packer.result.insert(best_packer.result.begin(), stub.begin(), stub.end());

        size_t best_size = best_packer.result.size();
        fprintf(stderr,
                "%u+%u=%u bytes after enabling DoVerySlow mode\n",
    ...
    for (size_t a = 0; a < best_packer.GammaSummary.size(); ++a)
        if (best_packer.GammaSummary[a])
        {
            ...
            fprintf(stderr,
                    "Sent gamma sequences that encode in %u bits: %u (range: %u..%u)\n",
                    (unsigned)a,
                    (unsigned)best_packer.GammaSummary[a],
                    min,
                    max);
            // 2 bits: 10,11
    ...
    if (nmismatch)
    {
        ok = false;
        fprintf(stderr,
                "%u failures, within %u..%u range (total size: %u)\n",
                nmismatch,
                firstfail,
                lastfail,
                r);
