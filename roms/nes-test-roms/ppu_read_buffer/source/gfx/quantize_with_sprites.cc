                                    ...
                                    for (unsigned c8 = c7; c8 < n; ++c8)
                                    {
                                        ...
                                        for (auto c : ind)
                                            for (unsigned n = 0; n < 4; ++n)
                                                outcome.c[n] += colors[c].c[n];
                                        emplace_back(
                                            std::move(Combination{{ind, ind + 8}, outcome}));
                                    }
        /**/
        /*
        for(unsigned c1=0; c1<n; ++c1)
...
std::array<int, n_elements>
DitherPixel(const Color& pixel, const PaletteType& palette, const Combinations& combinations)
{
    ...
    std::sort(result.begin(), result.end(), [&palette](unsigned a, unsigned b) {
        return palette[a].c[3] < palette[b].c[3];
    });
    return result;
}

/* Determine the amount of jitter that happens if pixels of these two colors are placed adjacent to
 * each others */
    ...
    for (unsigned color1 = 0; color1 < 128; ++color1)
        for (unsigned color2 = 0; color2 < 128; ++color2)
        {
            ...
            for (unsigned n = 0; n < 3; ++n)
            {
                ...
                c1[n] =
                    std::move(Color(DecodeNTSCsignal<4 * 8>(signal_buffers[n], 8, 8 + 12, HUE)));
                c2[n] =
                    std::move(Color(DecodeNTSCsignal<4 * 8>(signal_buffers[n], 16, 16 + 12, HUE)));
            }
            // The jitter strength is the difference between successive decodes of each pixel
            NTSCjitterStrengthMap[color1][color2] =
                c1[0].Difference(c1[1]) + c1[0].Difference(c1[2]) + c1[1].Difference(c1[2])
                + c2[0].Difference(c2[1]) + c2[0].Difference(c2[2]) + c2[1].Difference(c2[2]);
...
unsigned OrderedDitheringChoose(unsigned                         x,
                                unsigned                         y,
                                const std::array<int, n_colors>& mix,
...
template <int n_colors, typename PaletteType>
DitheredImageType DitherImage(const std::vector<std::vector<Color>>& pixels,
                              const PaletteType&                     palette,
                              ...
                              const std::string&                     fn)
{
    ...
    for (unsigned a = 0; a < (unsigned)palette.size(); ++a)
    {
        ...
        for (unsigned n = 0; n < 3; ++n)
            c(n) = std::pow(palette[a].c[n], 1 / 1.0);
    }
    ...
    double dithering_level = 0.09 * std::log((double)image.get_width() * image.get_height())
                             - 0.04 * std::log((double)palette.size()) + 0.001;
        ...
        for (int j = 0; j < 3; j++)
        {
            for (int k = 0; k < 3; k++)
            {
                sum += filter3_weights(i, j)(k) =
                    std::exp(-std::sqrt((double)((i - 1) * (i - 1) + (j - 1) * (j - 1)))
                             / (stddev * stddev));
            }
        }
    }
        ...
        for (int j = 0; j < 5; j++)
        {
            for (int k = 0; k < 3; k++)
            {
                sum += filter5_weights(i, j)(k) =
                    std::exp(-std::sqrt((double)((i - 2) * (i - 2) + (j - 2) * (j - 2)))
                             / (stddev * stddev));
            }
        }
    }
    ...
    array2d<vector_fixed<double, 3>>* filters[] =
        {NULL, &filter1_weights, NULL, &filter3_weights, NULL, &filter5_weights};
    std::printf("\rQuantizing...");
    std::fflush(stdout);
    spatial_color_quant(image,
                        *filters[filter_size],
                        quantized_image,
                        scolorq_palette,
                        coarse_variables,
                        ...
                        3,
                        1);
    // spatial_color_quant(image, filter3_weights, quantized_image, palette, coarse_variables, 0.05,
    // 0.02);

    for (unsigned y = 0; y < hei; ++y)
    {
        result[y].resize(wid);
        std::printf("\rFiltering... %d/%d", y, hei);
        std::fflush(stdout);
        ...
        for (unsigned x = 0; x < wid; ++x)
        {
            ...
            {
                i = cache.insert(i,
                                 std::move(cache_elem_type{
                                     rgb,
                                     DitherPixel<n_colors>(xspan[x], palette, combinations)}));
            }
        ...
        {
            Color c1(DecodeNTSCsignal<256 * 8>(ntsc_signal_1,
                                               x * 8,
                                               x * 8 + 12,
                                               ((phase1 * 8) % 12) + HUE));
            Color c2(DecodeNTSCsignal<256 * 8>(ntsc_signal_2,
                                               x * 8,
                                               x * 8 + 12,
                                               ((phase2 * 8) % 12) + HUE));
            auto& t = result[y][x];
    ...
    {
        result[y].resize(wid);
        std::printf("\rQuantizing... %d/%d", y, hei);
        std::fflush(stdout);
        const auto& xspan = pixels[y];
...
#pragma omp parallel for schedule(dynamic, 1)
    for (unsigned n = 0; n < 20; ++n)
    {
        ...
        combined_palette.insert(combined_palette.end(), palettes[p1].begin(), palettes[p1].end());
        if (n >= 4)
        ...
        dithereds[n] =
            std::move(DitherImage<n_colors>(pixels, combined_palette, combinations[n], fn));
...
#pragma omp parallel for schedule(static) reduction(+ : total_diff)
        for (unsigned y = 0; y < sy; y += 16)
        {
            for (unsigned py = 0; py < 16; py += 8)
                AttributeTable[y + py].resize(sx, 555);

            std::printf("\rChoosing tiles, %d/%d", y, sy);
            std::fflush(stdout);
        ...
        {
            std::printf("\rChoosing sprite %d/64...", spriteno);
            std::fflush(stdout);
...
#pragma omp parallel for schedule(static)
            for (unsigned y = 4; y <= sy - 16 - 8; ++y)
            {
                ...
                if (y < 64 && y + 16 >= 64)
                    continue; // Don't cross split-point 1
                if (y < 128 && y + 16 >= 128)
                    continue; // Don't cross split-point 2
                if (y < 184 && y + 16 >= 184)
                    continue; // Don't cross split-point 3
                    ...
                    for (unsigned spritecolor = 4; spritecolor < 8; ++spritecolor)
                    {
                        ...
                        for (unsigned py = 0; py < 16; ++py)
                            for (unsigned px = 0; px < 8; ++px)
                            {
                                ...
                                const auto& revised_pixel =
                                    std::get<1>(dithereds[q])[y + py][x + px];
                                const auto& original_pixel = pixels[y + py][x + px];
                                const auto& augmented_pixel =
                                    std::get<1>(dithereds[p])[y + py][x + px];
...
#pragma omp parallel for schedule(static) reduction(+ : total_diff) collapse(2)
        for (unsigned y = 0; y < sy; ++y)
            for (unsigned x = 0; x < sx; ++x)
            {
                ...
                for (unsigned spriteno = 0; spriteno < 64; ++spriteno)
                {
                    ...
                    if (sp.x <= x && sp.x + 8 > x && sp.y <= y && sp.y + 16 > y)
                    {
        ...
        if (total_diff < best_totaldiff)
        {
            ...
            for (unsigned p = 0; p < 8; ++p)
            {
                ...
                for (unsigned n = 0; n < 4; ++n)
                    std::fprintf(fp, "$%02X,", palettes[p][n].index);
                std::fprintf(fp, "\n");
            }
                ...
                for (unsigned x = 0; x < sx; x += 8)
                {
                    ...
                    for (unsigned py = 0; py < 8; ++py)
                        for (unsigned px = 0; px < 8; ++px)
                        {
                            ...
                            {
                                auto& sp = chosen_sprites[spriteno];
                                if (sp.x <= x + px && sp.x + 8 > x + px && sp.y <= y + py
                                    && sp.y + 16 > y + py)
                                {
                                    unsigned spritecolor = sp.attr;
                            ...
                            if (d >= 4)
                            {
                                if (d >= 5)
                                {
                                    // If the pixel is masked by sprite, try representing some of
                                    // the non-augmented graphics below it. This makes the image
                                    // look nice even when sprites are disabled or not supported.
        ...
        do
        {
            ...
            ++n_attempts;
        } while (new_color == 0x0F || new_color == 0x20 || new_color == 0x3D || new_color == 0x0D
                 || new_color == current_color || tried.find(attempt) != tried.end());
