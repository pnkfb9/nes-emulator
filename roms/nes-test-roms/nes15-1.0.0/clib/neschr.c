 ...
 */
static void
tile_from_img8(struct neschr_chr* chr, const struct img8_image* img, size_t dst, int sx, int sy);
...
struct neschr_chr* neschr_from_img8(const struct img8_image* img, int ver)
{
    ...
    if ((chr = neschr_new(img->w * img->h / (NESCHR_TILE_SIZE * NESCHR_TILE_SIZE)
                          * NESCHR_TILE_BYTES))
        == NULL)
    {
        return NULL;
...
int neschr_is_equal(const struct neschr_chr* chr1, const struct neschr_chr* chr2)
{
    assert(chr1 != NULL);
    assert(chr2 != NULL);
