...
struct img8_image* img8_read_pcx(struct binio_file* bf, struct img8_color* pal)
{
    ...
    if ((img = img8_new(header.x_max - header.x_min + 1, header.y_max - header.y_min + 1)) == NULL)
    {
        return NULL;
    }
...
static int read_pcx_header(struct binio_file* bf, struct pcx_header* h)
{
    ...
    if (h->man != 10 || h->ver != 5 || h->enc != 1 || h->bpp != 8 || h->num_planes != 1)
    {
        fprintf(stderr,
                "Error reading \"%s\", Not a valid 256 "
                "color PCX file\n",
                bf->name);
        return 0;
    }
